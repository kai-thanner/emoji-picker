use gtk::prelude::*;
use gtk::{
    ApplicationWindow, Box as GtkBox, Button, CheckButton, Dialog, Grid, Label,
...
pub fn zeige_einstellungsfenster(
	...
	let shortcut_button = Button::with_label("🔁 Tastenkürzel einrichten");
	shortcut_button.set_tooltip_text(Some("Hinweis:\nStandard: Super+.\nEigene Kombinationen bitte im Desktop-System einstellen."));
	{
	...
	let fenster_schliessen_checkbox = CheckButton::with_label("Fenster nach Auswahl eines Emojis automatisch schließen");
	fenster_schliessen_checkbox.set_active(einstellungen.fenster_schliessen.get());
	vbox.append(&fenster_schliessen_checkbox);

	// Drag-&-Drop Verhalten
	let drag_checkbox = CheckButton::with_label("Fenster bei Drag & Drop geöffnet lassen");
	drag_checkbox.set_active(einstellungen.fenster_offen_bei_drag.get());
	vbox.append(&drag_checkbox);

	// 🔠 Emoji-Größe
	let size_label = Label::new(Some("Emoji-Größe (px):"));
	...
	// 📖 Verlauf löschen
	let verlauf_button = Button::with_label("📖 Verlauf zurücksetzen");
	...
	// 🆗 Dialog OK / Schließen
	dialog.add_button("Abbrechen", ResponseType::Cancel);
	...
	dialog.connect_response(move |dialog, response| {
	    if response == ResponseType::Ok {
	        // einstellungen_neu.setup_erledigt.set(setup_checkbox.is_active());
